...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        return scheme_apply(self,
                            operands.map(lambda operand: scheme_eval(operand, env)), env)
        # END PROBLEM 5
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        lambdaprocedcure = LambdaProcedure(target.second, expressions.second, env)
        env.bindings[target.first] = lambdaprocedcure
        return target.first
        # END PROBLEM 10
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    lambdaprocedcure = LambdaProcedure(formals, expressions.second, env)
    return lambdaprocedcure
    # END PROBLEM 9
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        try:
            check_form(expression, 2, 2)
            formals = Pair(expression.first, formals)
            vals = Pair(scheme_eval(expression.second.first, env), vals)
        except SchemeError:
            break
...
def do_mu_form(expressions, env):
    ...
    check_formals(formals)
    # BEGIN PROBLEM 16
    muprocedcure = MuProcedure(formals, expressions.second)
    return muprocedcure
    # END PROBLEM 16
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN Extra Credit
            procedure = scheme_optimized_eval(expr.first, env, True)
            arguments = expr.second.map(lambda operand: scheme_optimized_eval(operand, env))
